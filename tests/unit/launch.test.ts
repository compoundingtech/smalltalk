...
describe('cmdLaunch — identity resolution', () => {
  ...
  it('falls back to $ST_AGENT when neither ST_* is set', async () => {
    const r = await cmdLaunch(
      baseInput({ env: { ST_AGENT: 'legacy-coord-bob' } }),
      ctx
    );
...
describe('cmdLaunch — pty.toml generation', () => {
  ...
  it('ST_ROOT propagation: no ST_ROOT/ST_ROOT in invoker env → no ST_ROOT in pty.toml', async () => {
  ...
  it('ST_ROOT propagation: invoker had legacy ST_ROOT set → pinned as ST_ROOT (canonicalized)', async () => {
    // A user still on the legacy ST_ROOT env var gets the pty
    // session env under the canonical ST_ROOT name, matching the
    // rest of the rename-cutover conventions. `input.stRoot` is
    // already resolved from ST_ROOT at the CLI layer.
    const legacyRoot = '/tmp/legacy-run/state';
    const r = await cmdLaunch(
      baseInput({
        identity: 'alice',
        env: { ST_ROOT: legacyRoot } as NodeJS.ProcessEnv,
        stRoot: legacyRoot,
      }),
      ctx
    );
  ...
  it('PTY_ROOT propagation: no ST_ROOT/ST_ROOT in invoker env → no PTY_ROOT in pty.toml', async () => {
  ...
  it('PTY_ROOT propagation: legacy ST_ROOT also produces the nested PTY_ROOT', async () => {
    // ST_ROOT is canonicalized to ST_ROOT throughout the launch;
  ...
  it('codex preview includes the `st ding` sidecar (ephemeral by default; no permanent tag)', async () => {
    ...
    // `st ding`. Hardcoding the legacy name into fresh pty.toml
  ...
  it('st.network tag emits for the DEFAULT network too (unlike ST_ROOT env)', async () => {
    ...
    const r = await cmdLaunch(
      // No ST_ROOT/ST_ROOT in env → default network.
      baseInput({ harness: 'claude', identity: 'alice', env: {} as NodeJS.ProcessEnv }),
      ctx
    );
    ...
    // Value is the resolved stRoot (a real absolute path, the test
