...
export interface AgentSummary {
   ...
   *  `--status` filters on. See `live`/`ageMs` for freshness. */
  status: State;
  name: string | null;
...
/**
 * #102: the status cell, annotated with freshness.
 *
 ...
 *
 * Shapes:
 *   available                  live, fresh
 *   available (stale 3m)       past the liveness window; the recorded
 *                              value is kept, because "was busy and went
 *                              quiet" and "cleanly offline" are
 *                              different facts
 *   unknown (was busy, 22m)    past the trust window too — derived state
 *                              is `unknown`, but we still say what it
 *                              last claimed
...
function renderStatusCell(m: AgentSummary, now = Date.now()): string {
  ...
  const age = formatAge(now - m.statusMtimeMs);
