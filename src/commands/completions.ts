// commands/completions.ts — print a shell completion script for `st`.
//
// st dispatches subcommands via a hand-written `switch` in cli.ts and
...
/**
 * The st command tree. Keep in sync with the `switch` dispatch in
...
const COMMANDS: readonly CommandSpec[] = [
  ...
  {
    name: 'init',
    desc: 'Write/merge .mcp.json so Claude loads st',
    takesPath: true,
    flags: [
...
function fishScript(): string {
  const out: string[] = [];
  out.push('# fish completions for st — generated by `st completions fish`.');
  out.push('# Regenerate with: st completions fish > ~/.config/fish/completions/st.fish');
  out.push('');
  out.push('complete -c st -e');
  out.push('complete -c st -f');
  out.push('');
  out.push('# Top-level subcommands');
  for (const c of COMMANDS) {
    for (const name of [c.name, ...(c.aliases ?? [])]) {
      out.push(
        `complete -c st -n __fish_use_subcommand -a ${name} -d ${q(c.desc)}`
      );
    }
  }
  ...
  for (const c of COMMANDS) {
    ...
    if (verbs.length > 0) {
      ...
      for (const v of verbs) {
        out.push(
          `complete -c st -n ${q(guard)} -a ${v.name} -d ${q(v.desc)}`
        );
      }
...
function fishEmitForNode(
  ...
  for (const f of node.flags ?? []) {
    if (f.values) {
      out.push(
        `complete -c st -n ${q(cond)} -l ${f.name} -x -a ${q(f.values.join(' '))} -d ${q(f.desc)}`
      );
    } else {
      out.push(`complete -c st -n ${q(cond)} -l ${f.name} -d ${q(f.desc)}`);
    }
  }
  if (node.positionalValues) {
    out.push(
      `complete -c st -n ${q(cond)} -x -a ${q(node.positionalValues.join(' '))} -d ${q('Value')}`
    );
  }
  if (node.takesPath) {
    out.push(`complete -c st -n ${q(cond)} -F`);
  }
}
...
function bashScript(): string {
  ...
  lines.push('# bash completion for st — generated by `st completions bash`.');
  lines.push('# Regenerate with: st completions bash > /etc/bash_completion.d/st');
  lines.push('_st() {');
...
function zshScript(): string {
  ...
  lines.push('#compdef st');
  lines.push('# zsh completion for st — generated by `st completions zsh`.');
  lines.push('# Regenerate with: st completions zsh > "${fpath[1]}/_st"');
  lines.push('_st() {');
...
/**
 * `st completions <shell>` — write a completion script for `shell` to
